(() => {
  ...
  // monta payload compatível com várias APis (value/amount/value_cents + product_name)
  const p = new URLSearchParams(location.search);
  ...
  const payload = {
    ...
    value: valorNum,
    amount: valorNum,
    value_cents: Math.round(valorNum * 100),
    customer_id: clienteId ? Number(clienteId) : undefined,
    cpf: cpf || undefined,
    status: 'pending' // deixa claro que entra na fila de aprovação
  };

  // helper pra chamar e logar a resposta com detalhes
  ...
  try {
    ...
    // tenta rota mais específica primeiro
    let data;
    try {
      data = await post(`${base}/api/client/orders`);
    } catch (e1) {
      if (String(e1.message||'').includes('404')) {
        // fallback para APIs que usam /api/orders
        data = await post(`${base}/api/orders`);
      } else {
        throw e1;
      }
    }
    ...
    console.error('[novo-pedido] erro ao criar pedido:', err); // veja a mensagem exata no Console
    out.textContent = err.message || 'Erro ao enviar pedido.';
  }
});
