...
// ---- UI instantânea vinda do evento (sem esperar a API)
// MOSTRA imediatamente a decisão (sem esperar a API)
...
function render(data){
  ...
  // ações nas parcelas (omitido aqui por brevidade — igual ao seu)
}
...
function setupRealtime() {
  try {
    ...
    bc.onmessage = (ev) => {
      const msg = ev?.data;
      if (msg?.type === 'order_decided') {
        // troca o texto imediatamente (verde/vermelho)...
        showDecisionFlash({ status: msg.status, product: msg.product, note: msg.note });
        // ...e sincroniza com a API logo em seguida
        loadOrderAlerts();
      }
    };
  } catch {}
...
// ===== Avatar (localStorage) ===== (idem ao seu)
(function(){
...
document.addEventListener('DOMContentLoaded', () => {
  setupRealtime();
  boot();                 // seu boot original que chama render/loadData etc.
});
