...
async function salvarCadastro() {
  ...
  try {
    ...
    if (r.status === 409) {
      // já existe -> pega id por cpf
      const got = await fetch(API + '/api/customers/by-cpf/' + cpf);
      if (!got.ok) throw new Error('CPF já existe e não consegui obter o cliente');
      const cli = await got.json();
      customerId = cli.id;
    } else if (r.ok) {
    ...
    // 2) cria contrato
    const base = parseFloat(valorBaseInput.value.replace(/\./g,'').replace(',','.')) || 0;
    const margin = parseFloat(margemInput.value) || 0;
    const parcelas = parseInt(parcelasInput.value, 10) || 1;
    const first_due = dataInput.value; // "YYYY-MM-DD"
    const tipo = tipoRadioVenda.checked ? 'venda' : 'negocio';

    r = await fetch(API + '/api/contracts', {
      method: 'POST',
      headers: { 'Content-Type':'application/json' },
      body: JSON.stringify({ customer_id: customerId, base, margin, parcelas, first_due, tipo })
    });

    if (!r.ok) {
      const err = await r.json().catch(()=>({}));
      throw new Error(err.error || 'Falha ao criar contrato');
    }

    alert('Cadastro salvo!');
    // opcional: abrir o portal desse cliente
    // window.open('acesso-cliente.html', '_blank');
