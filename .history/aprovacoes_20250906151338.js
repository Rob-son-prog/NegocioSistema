...
(function () {
  ...
  // --- tempo real: avisa outras abas (BroadcastChannel + localStorage fallback) ---
  function notifyClients(decision) {
    // decision = { id, status: 'approved'|'rejected' }
    try {
  ...
  document.addEventListener('click', async (e)=>{
    ...
    try{
      if (idA){
        ...
        await load();
        notifyClients({ id: idA, status: 'approved' });
      }
      if (idR){
        ...
        await load();
        notifyClients({ id: idR, status: 'rejected' });
      }
    }catch(err){
      alert(err.message);
    }
