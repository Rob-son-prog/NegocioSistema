...
function render(data){
  ...
  sorted.forEach((p, idx)=>{
    ...
    // <<< AQUI: só mostra "Baixar" se for ADMIN >>>
    const actions = `
    ...
    row.innerHTML = `
      <div class="parcel-col left">
        <div class="p-title">Parcela #${p.id || idx+1} — <strong>${val}</strong></div>
  ...
  el.parcelas.onclick = async (e)=>{
    ...
    const pixId = target.dataset.pix;
    const payId = target.dataset.pay;
    ...
    // Cliente nunca deve conseguir "baixar"
    if (payId && document.body.dataset.mode !== 'admin') return;
