...
import {
  ...
  findUserByEmail,
  createCustomer,     // precisa aqui
  normalizeCpf        // precisa aqui
} from './db.js';
...
// Rota nova e rota antiga (compatibilidade)
...
// handler de login do cliente por CPF (um lugar só)
function clientLoginHandler(req, res) {
  try {
    const { cpf } = req.body || {};
    if (!cpf) return res.status(400).json({ error: 'CPF é obrigatório' });

    const cli = findCustomerByCPF(cpf); // já normaliza dentro da função
    if (!cli) return res.status(404).json({ error: 'Cliente não encontrado' });

    const data = getPortalDataByCustomerId(cli.id);
...
app.post('/api/customers', (req, res) => {
  ...
  try {
    const info = createCustomer({ name, email, phone, cpf }); // já normaliza no db.js
    return res.status(201).json({
      id: info.lastInsertRowid,
      cpf: normalizeCpf(cpf) // devolve só números para você montar o link do portal
    });
  } catch (e) {
...
// mantém a rota que você já tinha
app.post('/api/client/login', clientLoginHandler);

// opcional: alias (se quiser chamar /portal/login do front)
app.post('/portal/login', clientLoginHandler);

// Buscar cliente por CPF (útil para o cadastro.html checar/evitar duplicidade)
...
app.post('/api/contracts', (req, res) => {
  try {
    const { customer_id, cpf, base, margin = 0, parcelas, first_due, tipo = 'negocio' } = req.body || {};
    if ((!customer_id && !cpf) || !base || !parcelas || !first_due) {
      return res.status(400).json({ error: 'Campos obrigatórios: (customer_id ou cpf), base, parcelas, first_due' });
    }

    // total com margem calculado no servidor
    const total = Number(base) * (1 + Number(margin || 0)/100);

    // resolve o customer_id via cpf se necessário
