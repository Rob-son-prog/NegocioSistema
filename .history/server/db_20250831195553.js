...
// --- já existentes (users, customers, contracts, installments) ---
// ... (deixe como está)

// ===== NOVA TABELA: orders (pedidos) =====
db.exec(`
CREATE TABLE IF NOT EXISTS orders (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  customer_id INTEGER,                   -- opcional, se quiser vincular ao cliente
  product TEXT NOT NULL,
  amount REAL NOT NULL,
  status TEXT NOT NULL DEFAULT 'pendente', -- pendente|aprovado|recusado
  created_at TEXT DEFAULT (datetime('now')),
...
// ===== FUNÇÕES DE ORDERS =====
export function createOrder({ customer_id = null, product, amount, status = 'pendente' }) {
  const stmt = db.prepare(`
    INSERT INTO orders (customer_id, product, amount, status)
    VALUES (?, ?, ?, ?)
  `);
  const info = stmt.run(customer_id, product, amount, status);
...
export function listPendingOrders() {
  return db.prepare(`SELECT * FROM orders WHERE LOWER(status) = 'pendente' ORDER BY id DESC`).all();
}
