...
// IMPORTA TUDO COMO NOMEADO (nada de default aqui)
import {
  db,
  createCustomer,
  listCustomers,
...
// =================== LOGIN DO CLIENTE (usa PHONE do customers) ==========
app.post('/api/auth/client-login', (req, res) => {
  try {
    const phoneRaw = String(req.body?.phone || req.body?.cpf || '').replace(/\D/g, '');
    if (!phoneRaw) return res.status(400).json({ error: 'phone/cpf inválido' });
    ...
    res.json({ token, client: { id: cli.id, name: cli.name, phone: cli.phone } });
  } catch (e) {
    console.error(e);
...
// =================== PORTAL DO CLIENTE (autenticado como cliente) =======
app.get('/api/portal/me', auth, (req, res) => {
  try {
    if (req.user.role !== 'client') return res.status(403).json({ error: 'forbidden' });

    const id = req.user.customerId;
    const cli = db.prepare(`SELECT id, name, phone FROM customers WHERE id = ?`).get(id);
    if (!cli) return res.status(404).json({ error: 'não encontrado' });

    const parcelas = listParcelasByCustomer(id);
    const toNum = v => Number(v || 0);
    const total = parcelas.reduce((s, p) => s + toNum(p.value ?? p.valor), 0);
