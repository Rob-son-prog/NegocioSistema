...
function ensureCPFColumn() {
  ...
  try {
    db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_customers_cpf ON customers(cpf)");
  } catch (e) {
    console.warn("Aviso ao criar índice único em cpf:", e.message);
  }

  const names = db.prepare("PRAGMA table_info(customers)").all().map(c => c.name);
  console.log("customers columns:", names.join(", "));
}
ensureCPFColumn();

// =================== FUNÇÕES AUXILIARES ===================

// Buscar cliente por CPF (normaliza para dígitos)
export function findCustomerByCPF(cpf) {
  const clean = String(cpf || '').replace(/\D/g, '');
  return db.prepare('SELECT * FROM customers WHERE cpf = ?').get(clean);
}
