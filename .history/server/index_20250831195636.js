...
import express from 'express';
import cors from 'cors';
// se você usa jwt, deixe como está
// import jwt from 'jsonwebtoken';

import {
  // ... (suas funções antigas)
  createOrder,
  listOrders,
  listPendingOrders,
  updateOrderStatus,
...
// Se você já tem um middleware de auth, use-o aqui.
// Para não dar 403 enquanto ajusta, pode liberar essas rotas
// ou validar só se tiver token válido. Exemplo simples:
//
// function authOptional(req, res, next) {
//   // Se quiser exigir token, troque por uma verificação real.
//   // Por enquanto, deixa passar (evita 403 no teste).
//   next();
// }
...
app.post('/api/orders', /*authOptional,*/ (req, res) => {
  try {
    const { product, amount, status = 'pendente', customer_id = null } = req.body || {};
    if (!product || !amount || isNaN(Number(amount))) {
      return res.status(400).json({ error: 'product e amount são obrigatórios' });
    }
    const order = createOrder({ product: String(product), amount: Number(amount), status, customer_id });
...
app.get('/api/orders', /*authOptional,*/ (_req, res) => {
  try {
...
app.get('/api/orders/pending', /*authOptional,*/ (_req, res) => {
  try {
    const list = listPendingOrders();
    res.json(list);
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Falha ao listar pendentes' });
  }
});

app.post('/api/orders/:id/approve', /*authOptional,*/ (req, res) => {
  try {
    const id = Number(req.params.id);
    const order = updateOrderStatus(id, 'aprovado');
    if (!order) return res.status(404).json({ error: 'Pedido não encontrado' });
    res.json(order);
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Falha ao aprovar pedido' });
  }
});

app.post('/api/orders/:id/reject', /*authOptional,*/ (req, res) => {
  try {
    const id = Number(req.params.id);
    const order = updateOrderStatus(id, 'recusado');
    if (!order) return res.status(404).json({ error: 'Pedido não encontrado' });
    res.json(order);
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Falha ao recusar pedido' });
  }
});
