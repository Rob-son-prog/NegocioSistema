...
/* Pedidos do cliente para aprovação */
...
/**
 * Lista pedidos.
 * Se `status` for null/undefined => lista todos.
 * Se informado => 'pendente' | 'aprovado' | 'recusado'
 */
export function listOrderRequests(status = null) {
  const sql = `
    SELECT o.id, o.product, o.amount, o.status, o.created_at, o.decided_at, o.decision_note,
...
export function setOrderRequestStatus(id, status, note = null) {
  const nstatus = String(status).toLowerCase(); // 'aprovado' | 'recusado' | 'pendente'
  return db.prepare(`
...
export function deleteOrderRequest(id) {
  return db.prepare(`DELETE FROM order_requests WHERE id = ?`).run(id);
}
