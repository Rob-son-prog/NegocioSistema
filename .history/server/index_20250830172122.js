...
function authClient(req, res, next) {
  ...
  try {
    ...
    if (payload.role !== 'client') return res.status(403).json({ error: 'Permissão negada' });
    req.client = payload; // { role:'client', customerId }
    next();
  } catch {
...
app.post('/api/client/login', (req, res) => {
  try {
    ...
    const token = jwt.sign({ role: 'client', customerId: cli.id }, JWT_SECRET, { expiresIn: '12h' });
...
app.post('/api/customers', (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: info.lastInsertRowid, cpf: normalizeCpf(cpf) });
  } catch (e) {
    if (String(e).includes('UNIQUE')) return res.status(409).json({ error: 'CPF já cadastrado' });
    console.error(e); res.status(500).json({ error: 'Erro ao salvar cliente' });
  }
});
...
app.post('/api/contracts', (req, res) => {
  try {
    ...
    const contract_id = createContractAndInstallments({
      customer_id: cid, total, parcelas: Number(parcelas), first_due, tipo
    });
...
app.post('/api/orders', authClient, (req, res) => {
  try {
    ...
    // better-sqlite3 .run() retorna { lastInsertRowid }
    res.status(201).json({ id: info.lastInsertRowid ?? info });
  } catch (e) { console.error(e); res.status(500).json({ error: 'internal' }); }
});

// Admin lista pedidos (padrão: pendente; ou ?status=aprovado|recusado; vazio => todos)
...
// Admin aprova/reprova (opcional 'note' no body)
...
// Cliente vê os pedidos dele (para alertas no portal)
