...
// --- ORDERS (pedidos do cliente p/aprovação) -------------------------------
// Tabela (crie se ainda não tiver)
db.exec(`
CREATE TABLE IF NOT EXISTS order_requests (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  customer_id INTEGER NOT NULL REFERENCES customers(id) ON DELETE CASCADE,
...
export function createOrderRequest({ customer_id, product, amount }) {
  return db.prepare(`
    INSERT INTO order_requests (customer_id, product, amount)
    VALUES (?, ?, ?)
  `).run(customer_id, product, amount);
}

export function listOrderRequests(limit = 200) {
  return db.prepare(`
...
export function setOrderRequestStatus(id, status, note = null) {
  const nstatus = String(status).toLowerCase(); // approved | rejected
  return db.prepare(`
    UPDATE order_requests
