...
import {
  // ...suas outras funções
  markInstallmentPaid,
  sumReceivedForMonth,   // <-- adicione
  sumReceivedCurrentMonth // <-- opcional (se quiser usar)
} from './db.js';
...
import {
  ...
  // admin - edição/exclusão (certifique-se de exportar no db.js)
  deleteContract,
  updateInstallment,
  deleteInstallment,
} from './db.js';
...
// KPI: recebidos no mês (pelo paid_at)
app.get('/api/kpis/monthly', (req, res) => {
  try {
    const now = new Date();
    const y = Number(req.query.year  || now.getUTCFullYear());
    const m = Number(req.query.month || (now.getUTCMonth() + 1));
...
app.post('/api/customers', (req, res) => {
  ...
  try {
    const info = createCustomer({ name, email, phone, cpf }); // normaliza dentro do db.js
...
app.get('/api/kpis/recebidos-mes', (req, res) => {
  try {
    const now = new Date();
    const ym = now.toISOString().slice(0,7); // "2025-08"

    // soma parcelas com status PAGO e paid_at dentro do mês atual
    const sql = `
    ...
    `;
    const row = db.prepare(sql).get(ym);
    res.json({ total: row?.total || 0 });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Falha ao calcular recebidos' });
  }
});

// KPI: recebidos no mês
app.get('/api/kpis/recebidos-mes', (req, res) => {
  try {
    const now = new Date();
